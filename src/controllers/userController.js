...
const loginUser = async function(req, res) {
    try {
        ...
        if (!email) return res.status(400).send({status: false,message: "Email Is Not Given"})
        let password = req.body.password;
        if (!password) return res.status(400).send({status: false,message: "Password Is Not Given"})
